...
const FIGMA_SEEK_BAR_Y_OFFSET_FROM_PANEL_CENTER_PX =
  FIGMA_PANEL_HEIGHT_PX / 2 - 54;
...
const WORLD_SEEK_BAR_PROGRESS_HEIGHT =
  (FIGMA_SEEK_BAR_HEIGHT_PX - 1) * SCALE_FACTOR;
const WORLD_SEEK_BAR_Y_OFFSET =
  FIGMA_SEEK_BAR_Y_OFFSET_FROM_PANEL_CENTER_PX * SCALE_FACTOR;
const WORLD_SEEK_BAR_HIT_AREA_HEIGHT_MULTIPLIER = 5;
const WORLD_SEEK_BAR_HIT_AREA_HEIGHT =
  WORLD_SEEK_BAR_TRACK_HEIGHT * WORLD_SEEK_BAR_HIT_AREA_HEIGHT_MULTIPLIER;

const PANEL_TEXTURE_WIDTH = 1024;
const PANEL_TEXTURE_HEIGHT = Math.round(
  PANEL_TEXTURE_WIDTH * (FIGMA_PANEL_HEIGHT_PX / FIGMA_PANEL_WIDTH_PX)
);
...
let vrPlayPauseButtonMesh,
  vrPlayPauseButtonCanvas,
  vrPlayPauseButtonContext,
  vrPlayPauseButtonTexture;
let vrRewindButtonMesh;
let vrForwardButtonMesh;
let vrExitButtonMesh;
let vrVolumeButtonMesh,
  vrVolumeButtonCanvas,
  vrVolumeButtonContext,
  vrVolumeButtonTexture;
...
const startVideo = () => {
  ...
  if (!videoElement) {
    console.error(
      "CRITICAL_ERROR_DOM: Essential HTML element (video) not found."
    );
    return;
  }
  ...
  if (navigator.xr) {
    navigator.xr
      ...
      .catch((err) => {
        ...
        playBtn.dataset.xrSupported = "false";
        // EnaVble button for regular video playback when VR check fails
        playBtn.disabled = false;
...
function init() {
  try {
    ...
    if (!renderer || !renderer.isWebGLRenderer) {
      throw new Error(
        "Failed to create WebGLRenderer or it's not a valid Three.js renderer type."
      );
    }
    ...
    gl.canvas.addEventListener(
      ...
      (event) => {
        ...
        console.error(
          "CONTEXT_EVENT: WebGL Context Lost! xrSession active?",
          !!xrSession,
          event
        );
        if (xrSession) {
    ...
    gl.canvas.addEventListener(
      ...
      (event) => {
        ...
        if (
          ...
          console.log(
            "Re-initialized video texture after context restoration during VR."
          );
        }
      },
      false
    ...
    vr180Mesh.onBeforeRender = function (
      renderer,
      scene,
      activeCamera,
      geometry,
  ...
  try {
    ...
    const textureCornerRadius =
      FIGMA_CORNER_RADIUS_PX * (PANEL_TEXTURE_WIDTH / FIGMA_PANEL_WIDTH_PX);
    drawRoundedRect(
      panelCtx,
      0,
      0,
    ...
    const videoTitle =
      ...
      videoElement
        ...
        .replace(/-/g, " ") ||
      "Video Title";
    const titleFontSizeTexturePx = Math.round(
      FIGMA_TITLE_FONT_SIZE_PX * (PANEL_TEXTURE_HEIGHT / FIGMA_PANEL_HEIGHT_PX)
    );
    ...
    const titleMarginTopTexturePx =
      FIGMA_TITLE_MARGIN_TOP_PX *
      (PANEL_TEXTURE_HEIGHT / FIGMA_PANEL_HEIGHT_PX);
    panelCtx.fillText(
      videoTitle,
      PANEL_TEXTURE_WIDTH / 2,
      titleMarginTopTexturePx
    );
    ...
    const seekBarTrackMaterial = new THREE.MeshBasicMaterial({
      color: 0x767676,
      transparent: true,
      opacity: 0,
    });
    const seekBarTrackGeometry = new THREE.PlaneGeometry(
      WORLD_SEEK_BAR_WIDTH,
      WORLD_SEEK_BAR_TRACK_HEIGHT
    );
    seekBarTrackMesh = new THREE.Mesh(
      seekBarTrackGeometry,
      seekBarTrackMaterial
    );
    ...
    const seekBarProgressMaterial = new THREE.MeshBasicMaterial({
      color: 0xffffff,
      transparent: true,
      opacity: 0,
    });
    const seekBarProgressGeometry = new THREE.PlaneGeometry(
      WORLD_SEEK_BAR_WIDTH,
      WORLD_SEEK_BAR_PROGRESS_HEIGHT
    );
    seekBarProgressMesh = new THREE.Mesh(
      seekBarProgressGeometry,
      seekBarProgressMaterial
    );
    ...
    const seekBarHitAreaMaterial = new THREE.MeshBasicMaterial({
      visible: false,
      transparent: true,
      opacity: 0,
    });
    seekBarHitAreaMesh = new THREE.Mesh(
      seekBarHitAreaGeometry,
      seekBarHitAreaMaterial
    );
    ...
    const playPauseButtonWorldSize =
      FIGMA_PLAYPAUSE_BUTTON_SIZE_PX * SCALE_FACTOR;
    const playPauseButtonGeometry = new THREE.PlaneGeometry(
      playPauseButtonWorldSize,
      playPauseButtonWorldSize
    );
    vrPlayPauseButtonMesh = new THREE.Mesh(
      playPauseButtonGeometry,
      playPauseButtonMaterial
    );
    ...
    const worldPlayPauseButtonOffsetX =
      (figmaPlayPauseButtonCenterX - FIGMA_PANEL_WIDTH_PX / 2) * SCALE_FACTOR;
    const worldPlayPauseButtonOffsetY =
      -(figmaPlayPauseButtonCenterY - FIGMA_PANEL_HEIGHT_PX / 2) * SCALE_FACTOR;
    vrPlayPauseButtonMesh.position.set(
      worldPlayPauseButtonOffsetX,
      worldPlayPauseButtonOffsetY,
      0.02
    );
    ...
    const rewindButtonTexture = createButtonTexture(
      rewindSVGPath,
      "#ffffff",
      "rgba(0,0,0,0)",
      ...
      0,
      true,
      FIGMA_REWIND_BUTTON_SIZE_PX
    );
    const rewindButtonMaterial = new THREE.MeshBasicMaterial({
      map: rewindButtonTexture,
      transparent: true,
      opacity: 0,
      depthWrite: false,
    });
    const rewindButtonGeometry = new THREE.PlaneGeometry(
      rewindButtonWorldSize,
      rewindButtonWorldSize
    );
    vrRewindButtonMesh = new THREE.Mesh(
      rewindButtonGeometry,
      rewindButtonMaterial
    );
    ...
    const worldRewindButtonOffsetX =
      (figmaRewindButtonCenterX - FIGMA_PANEL_WIDTH_PX / 2) * SCALE_FACTOR;
    const worldRewindButtonOffsetY =
      -(figmaRewindButtonCenterY - FIGMA_PANEL_HEIGHT_PX / 2) * SCALE_FACTOR;
    vrRewindButtonMesh.position.set(
      worldRewindButtonOffsetX,
      worldRewindButtonOffsetY,
      0.02
    );
    ...
    const forwardButtonTexture = createButtonTexture(
      forwardSVGPath,
      "#ffffff",
      "rgba(0,0,0,0)",
      VR_BUTTON_TEXTURE_SIZE,
      0,
      true,
      FIGMA_FORWARD_BUTTON_SIZE_PX
    );
    const forwardButtonMaterial = new THREE.MeshBasicMaterial({
      map: forwardButtonTexture,
      transparent: true,
      opacity: 0,
      depthWrite: false,
    });
    const forwardButtonGeometry = new THREE.PlaneGeometry(
      forwardButtonWorldSize,
      forwardButtonWorldSize
    );
    vrForwardButtonMesh = new THREE.Mesh(
      forwardButtonGeometry,
      forwardButtonMaterial
    );
    ...
    const worldForwardButtonOffsetX =
      (figmaForwardButtonCenterX - FIGMA_PANEL_WIDTH_PX / 2) * SCALE_FACTOR;
    const worldForwardButtonOffsetY =
      -(figmaForwardButtonCenterY - FIGMA_PANEL_HEIGHT_PX / 2) * SCALE_FACTOR;
    vrForwardButtonMesh.position.set(
      worldForwardButtonOffsetX,
      worldForwardButtonOffsetY,
      0.02
    );
    ...
    const exitButtonTexture = createButtonTexture(
      ...
      0,
      true,
      FIGMA_EXIT_BUTTON_SIZE_PX
    );
    const exitButtonMaterial = new THREE.MeshBasicMaterial({
      map: exitButtonTexture,
      transparent: true,
      opacity: 0,
      depthWrite: false,
    });
    const exitButtonGeometry = new THREE.PlaneGeometry(
      exitButtonWorldSize,
      exitButtonWorldSize
    );
    ...
    const worldExitButtonOffsetX =
      (figmaExitButtonCenterX - FIGMA_PANEL_WIDTH_PX / 2) * SCALE_FACTOR;
    const worldExitButtonOffsetY =
      -(figmaExitButtonCenterY - FIGMA_PANEL_HEIGHT_PX / 2) * SCALE_FACTOR;
    vrExitButtonMesh.position.set(
      worldExitButtonOffsetX,
      worldExitButtonOffsetY,
      0.02
    );
    ...
    const volumeButtonMaterial = new THREE.MeshBasicMaterial({
      map: vrVolumeButtonTexture,
      transparent: true,
      opacity: 0,
      depthWrite: false,
    });
    const volumeButtonWorldSize = FIGMA_VOLUME_BUTTON_SIZE_PX * SCALE_FACTOR;
    const volumeButtonGeometry = new THREE.PlaneGeometry(
      volumeButtonWorldSize,
      volumeButtonWorldSize
    );
    vrVolumeButtonMesh = new THREE.Mesh(
      volumeButtonGeometry,
      volumeButtonMaterial
    );
    ...
    const worldVolumeButtonOffsetX =
      (figmaVolumeButtonCenterX - FIGMA_PANEL_WIDTH_PX / 2) * SCALE_FACTOR;
    const worldVolumeButtonOffsetY =
      -(figmaVolumeButtonCenterY - FIGMA_PANEL_HEIGHT_PX / 2) * SCALE_FACTOR;
    vrVolumeButtonMesh.position.set(
      worldVolumeButtonOffsetX,
      worldVolumeButtonOffsetY,
      0.02
    );
...
function updateVRPlayPauseButtonIcon() {
  ...
  ctx.translate(
    (canvas.width - scaledIconSize) / 2,
    (canvas.height - scaledIconSize) / 2
  );
  ctx.scale(iconScale, iconScale);
  if (video.paused || video.ended) {
    const playPath = new Path2D(
      "M32.3859 20.4038C33.4457 21.204 33.4457 22.796 32.3859 23.5962L18.2051 34.3026C16.8874 35.2974 15 34.3575 15 32.7064V11.2936C15 9.64253 16.8874 8.70258 18.2051 9.69741L32.3859 20.4038Z"
    );
    ctx.fill(playPath);
  } else {
    const rectX1 = 16;
...
function updateVRVolumeButtonIcon() {
  ...
  const pathOffsetX =
    (canvas.width - svgPathNaturalViewBoxSize * pathRenderScale) / 2;
  const pathOffsetY =
    (canvas.height - svgPathNaturalViewBoxSize * pathRenderScale) / 2;
...
function updateSeekBarAppearance() {
  if (
    ...
    seekBarProgressMesh.position.x =
      -WORLD_SEEK_BAR_WIDTH / 2 + (WORLD_SEEK_BAR_WIDTH * progress) / 2;
...
function updateCameraRotation() {
  ...
  cameraRotation.pitch = Math.max(
    -MAX_PITCH,
    Math.min(MAX_PITCH, cameraRotation.pitch)
  );

  // Constrain yaw (horizontal rotation) to VR180 content boundaries
  cameraRotation.yaw = Math.max(
    -MAX_YAW,
    Math.min(MAX_YAW, cameraRotation.yaw)
  );
...
function position2DControlPanel() {
  ...
  const topPosition =
    canvasRect.bottom - containerRect.top - bottomOffset - panelHeight;
    ...
    actualSessionToggle().catch((err) => {
      ...
      if (xrSession) {
        ...
        sessionToClean
          .end()
          .catch((e) => {})
          .finally(() => onVRSessionEnd({ session: sessionToClean }));
      } else {
...
function onSelectStartVR(event) {
  ...
  if (directIntersects.length > 0) {
    ...
    if (firstIntersected.name === "vrPlayPauseButton") {
      ...
      firstIntersected.name === "seekBarHitArea" &&
      video &&
      isFinite(video.duration)
    ) {
      showPanel();
      const localPoint = seekBarTrackMesh.worldToLocal(
        intersectionPoint.clone()
      );
      const normalizedPosition =
        (localPoint.x + WORLD_SEEK_BAR_WIDTH / 2) / WORLD_SEEK_BAR_WIDTH;
      const newTime =
        Math.max(0, Math.min(1, normalizedPosition)) * video.duration;
      video.currentTime = newTime;
      updateSeekBarAppearance();
    } else if (
      firstIntersected.name === "vrControlPanelBackground" ||
      firstIntersected.name === "vr180Mesh"
    ) {
      if (vrControlPanel && vrControlPanel.visible && panelOpacity > 0.01)
        hidePanel();
      else showPanel();
    } else {
      if (vrControlPanel && vrControlPanel.visible && panelOpacity > 0.01)
...
async function actualSessionToggle() {
  if (!renderer || !renderer.isWebGLRenderer) {
    console.error(
      "CRITICAL_ERROR: actualSessionToggle: renderer is NOT a WebGLRenderer or is null!",
      renderer
    );
    return;
  }
  ...
  if (xrSession) {
    ...
    try {
      ...
      if (!session) {
        throw new Error("requestSession returned no session.");
      }
      ...
      if (video && (video.paused || video.ended)) {
        try {
          ...
          console.error(
            "Failed to play video after obtaining XR session:",
            playError
          );
        }
      }
      ...
      const sessionStartError =
        "XR_ERROR: Failed to start VR session: " + (err.message || String(err));
...
function onVRSessionEnd(event) {
  ...
  if (xrSession === endedSession || xrSession === null) {
    ...
    console.warn(
      "onVRSessionEnd: Global xrSession was different from the endedSession. Global xrSession:",
      xrSession,
      "Ended session:",
      endedSession
    );
...
function handleControllerInteractions() {
  if (!renderer || !renderer.xr || !renderer.xr.isPresenting || !controller1)
    return;
}
...
function renderXR(timestamp, frame) {
  ...
  if (!renderer || !renderer.xr || !renderer.xr.isPresenting) {
    console.warn(
      "renderXR called but not in a valid XR presenting state. Stopping loop."
    );
    isXrLoopActive = false;
  ...
  if (frameCounter > 0 && frameCounter % 3600 === 0) {
    ...
    if (error !== gl.NO_ERROR) {
      console.error(
        `WEBGL_ERROR_IN_RENDER_LOOP (F${frameCounter}):`,
        error,
        gl.enumToString ? gl.enumToString(error) : error
      );
    }
  }
  try {
    ...
    if (sessionToCloseOnError) {
      ...
      sessionToCloseOnError
        .end()
        .catch((e) => {
          console.error(
            "Error trying to end session after render loop crash:",
            e
          );
        })
        .finally(() => {
